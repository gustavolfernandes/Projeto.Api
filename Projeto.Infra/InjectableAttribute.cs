            ...
            foreach (var attr in attrs)
            {
                switch (attr.LifeTime)
                {
                    case LifeTime.Scoped:
                        if (attr.Abstraction is null) services.AddScoped(type, type);
                        else services.AddScoped(attr.Abstraction, type);
                        break;

                    case LifeTime.Singleton:
                        if (attr.Abstraction is null) services.AddSingleton(type, type);
                        else services.AddSingleton(attr.Abstraction, type);
                        break;

                    case LifeTime.Transient:
                        if (attr.Abstraction is null) services.AddTransient(type, type);
                        else services.AddTransient(attr.Abstraction, type);
                        break;
